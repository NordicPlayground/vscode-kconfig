...
def test_init_no_workspace():
    rsp = request(
        "initialize", {"rootUri": str(Uri.file(zephyr_root)), "workspaceFolders": None}
    )
...
def test_initialized_notification():
    ...
    caps = [
        req.params["registrations"][0]
        for req in requests
        if req.method == "client/registerCapability"
    ]
    watchers = [
        reg["registerOptions"]["watchers"][0]
        for reg in caps
    ...
    assert watchers == [
        {"globPattern": "**/Kconfig*", "kind": 7},
        {"globPattern": "**/edt.pickle", "kind": 7},
    ]
...
def test_parse_context():
    ...
    notifications.clear()
    assert (
        request("kconfig/setMainBuild", {"uri": str(Uri.file(build_folder))}).error
        == None
    )
    assert len(notifications) > 0

    diags = [
        n.params for n in notifications if n.method == "textDocument/publishDiagnostics"
    ]
    assert len(diags) > 0
...
def test_workspace_symbols():
    ...
    assert [r["name"] for r in rsp.result] == [
        ...
        "CONFIG_OPTION_3",
        "CONFIG_HIDDEN_ENTRY",
        "CONFIG_ENTRY_INSERTED",
        "CONFIG_BOARD_SPECIFIC_ENTRY",
        "CONFIG_plain_ENTRY",
        "CONFIG_relative_ENTRY",
        "CONFIG_optional_ENTRY",
        "CONFIG_relative_optional_ENTRY",
        "CONFIG_OPTION_4",
    ]
...
def test_completion():
    ...
    rsp = request(
        ...
        {
            "textDocument": {"uri": str(Uri.file(path.join(zephyr_root, "prj.conf")))},
    ...
    rsp = request(
        "textDocument/completion",
        {
            ...
            "position": Position(
                0, 10
            ).__dict__,  # @ CONFIG_TES, should yield CONFIG_TEST_* entries
        },
    )
    ...
    rsp = request(
        "textDocument/completion",
        {
            ...
            "position": Position(
                0, 4
            ).__dict__,  # @ CONF, should act like root completion
        },
    )
    ...
    assert [i["insertText"] for i in rsp.result["items"]] == [
...
def test_watcher():
    ...
    assert (
        len([n for n in notifications if n.method == "textDocument/publishDiagnostics"])
        > 0
    )
...
def test_hover():
    ...
    rsp = request(
        "textDocument/hover",
        {
            "textDocument": {"uri": str(Uri.file(path.join(zephyr_root, "Kconfig")))},
    ...
    rsp = request(
        "textDocument/hover",
        {
            "textDocument": {"uri": str(Uri.file(path.join(zephyr_root, "source.c")))},
...
def test_code_action():
    ...
    assert [r["title"] for r in rsp.result] == [
        "Enable CONFIG_TEST_ENTRY2 to resolve dependency",
        "Remove entry",
        "Remove entry",
    ]
...
def test_parser_errors():
    ...
    assert (
        conf_diags["diagnostics"][-1]["message"]
        == "ignoring malformed line 'INVALID_TOKEN'"
    )
...
def test_set_val():
    ...
    assert request("kconfig/search", {"query": "TEST_ENTRY1"}).result["symbols"][0][
        "visible"
    ]
    ...
    # Invisible again:
    assert not request("kconfig/search", {"query": "TEST_ENTRY1"}).result["symbols"][0][
        "visible"
    ]
